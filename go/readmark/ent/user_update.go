...
func (uuo *UserUpdateOne) SetFromOpt(opt *UserOpt) *UserUpdateOne {
	if opt.ID != nil && *opt.ID > 0 {
		uuo.mutation.SetID(*opt.ID)
	}
	uuo.SetNillableAccount(opt.Account)
	uuo.SetNillablePassword(opt.Password)
	return uuo
}
